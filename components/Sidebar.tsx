    ...
    <>
      ...
      `}>
        <div className="flex justify-between items-center shrink-0">
          ...
          <button onClick={onClose} className="hover:text-cyan-500 transition-colors p-2 text-slate-400">
        ...
        <nav className="flex flex-col gap-2 shrink-0">
          <p className="text-[10px] font-black text-slate-500 uppercase tracking-[0.3em] ml-2 mb-2">Entregadores</p>
          <SidebarButton onClick={() => onOpenModal('register')} label="Novo Cadastro" icon="plus-circle" color="cyan" />
          <SidebarButton onClick={() => onOpenModal('delete-driver')} label="Gestão de Banco" icon="database" color="purple" />
          {isAdmin && <SidebarButton onClick={() => onOpenModal('register-tenant')} label="Novo Inquilino" icon="users" color="red" />}
          {isAdmin && <SidebarButton onClick={() => onOpenModal('manage-pins')} label="Gestão de PINS" icon="lock" color="emerald" />}

          <p className="text-[10px] font-black text-slate-500 uppercase tracking-[0.3em] ml-2 mt-6 mb-2">Monitoramento</p>
        ...
        <div className="mt-auto flex flex-col gap-6 shrink-0 pt-4">
          {lastExit && (
            <div className={`p-5 rounded-3xl border transition-all ${isDarkMode ? 'bg-slate-800/40 border-slate-700' : 'bg-white border-slate-100 shadow-sm'}`}>
              ...
              <p className="font-black text-base tracking-tight mb-3 truncate text-slate-400">{lastExit.name}</p>
              <div className="flex flex-col gap-2 text-[9px] font-bold mono uppercase">
                ...
                <div className="flex justify-between items-center border-t border-slate-500/10 pt-2 text-slate-500">
          ...
          <div className="flex flex-col gap-2">
            <button
              ...
              className={`flex items-center justify-between p-4 rounded-2xl border transition-all hover:bg-slate-500/5 ${isDarkMode ? 'bg-slate-800 border-slate-700' : 'bg-white border-slate-100'}`}
            >
            ...
            <button
              onClick={onLogout}
              className={`flex items-center gap-4 p-4 rounded-2xl border transition-all hover:bg-red-500/10 hover:border-red-500/50 group ${isDarkMode ? 'bg-slate-800 border-slate-800 text-slate-400' : 'bg-white border-slate-100 text-slate-500'}`}
            >
