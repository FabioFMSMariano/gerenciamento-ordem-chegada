  ...
  return (
    <div className={`flex flex-col gap-4 h-full animate-in fade-in slide-in-from-bottom-4 duration-1000 relative`}>
      ...
      <div className="flex items-center gap-2 px-2 shrink-0">
        <div className="w-1.5 h-6 bg-gradient-to-b from-cyan-400 to-blue-600 rounded-full shadow-[0_0_10px_rgba(0,255,255,0.4)]" />
      ...
      <div className="flex flex-col gap-3 overflow-hidden pr-1">
        {recentLogs.length > 0 ? (
          recentLogs.map((log) => (
            <div 
              key={log.id} 
              className={`p-5 rounded-[28px] border transition-all hover:border-cyan-500/50 relative overflow-hidden group ${
                isDarkMode 
            ...
            >
              ...
              <div className="flex justify-between items-start mb-4 relative z-10">
                ...
                <span className={`px-3 py-1.5 rounded-xl text-[9px] font-black mono whitespace-nowrap shadow-sm ${
              ...
              <div className="flex flex-col gap-3 relative z-10">
                <div className="flex items-center gap-2">
                  <button 
                    ...
                    className={`flex-1 flex items-center justify-between px-4 py-3 rounded-2xl border transition-all active:scale-95 ${
                      isDarkMode 
                        ? 'bg-slate-950 border-white/5 hover:bg-cyan-500/5' 
                        : 'bg-slate-50 border-slate-100 hover:bg-cyan-50'
                    }`}
                  >
                    <span className="text-[11px] font-black mono text-cyan-500 uppercase flex items-center gap-2">
                  ...
                  <div className={`px-4 py-3 rounded-2xl border flex items-center gap-2 shadow-sm shrink-0 min-w-[75px] justify-center ${
                    isDarkMode 
          ...
          <div className={`p-10 rounded-2xl border-2 border-dashed flex flex-col items-center justify-center gap-3 ${
            isDarkMode ? 'border-slate-800 bg-slate-900/20 opacity-30' : 'border-slate-200 bg-slate-50 opacity-50'
          }`}>
