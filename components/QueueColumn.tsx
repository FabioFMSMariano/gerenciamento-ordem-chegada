...
const QueueColumn: React.FC<QueueColumnProps> = ({ 
  ...
  return (
    <div className="w-full flex flex-col gap-6">
      {copyFeedback && (
        <div className="fixed top-12 left-1/2 -translate-x-1/2 z-[100] bg-cyan-400 text-slate-900 px-8 py-3 rounded-2xl text-xs font-black animate-in fade-in slide-in-from-top duration-300 shadow-2xl border border-white/20">
          {copyFeedback}
        </div>
      )}
      ...
      <div className={`p-8 rounded-[40px] border-2 transition-all ${isDarkMode ? 'bg-slate-900/60 border-white/5 shadow-2xl' : 'bg-white border-slate-200 shadow-xl'}`}>
        <div className="flex items-center justify-between mb-8">
          <div className="flex items-center gap-4">
            <div className={`w-4 h-4 rounded-full ${periodColors.bg} animate-pulse shadow-[0_0_10px_rgba(251,191,36,0.5)]`} />
            <h2 className={`text-2xl font-black uppercase tracking-[0.2em] mono ${periodColors.text}`}>{period}</h2>
          </div>
          <button onClick={onAddClick} className={`p-4 rounded-2xl transition-all hover:scale-110 active:scale-95 ${isDarkMode ? 'bg-cyan-500 text-slate-950 shadow-[0_0_15px_rgba(6,182,212,0.4)]' : 'bg-cyan-600 text-white shadow-lg'}`}>
                    ...
                    <div className="flex gap-6 items-center flex-1 truncate">
                      ...
                      <div className="flex flex-col truncate">
                        ...
                        <div className={`text-[10px] uppercase font-black mono flex flex-wrap items-center gap-x-3 gap-y-1 mt-1 
                          ...
                          <span className={isFirst ? (isMorning ? 'text-blue-200' : 'text-emerald-200') : 'text-cyan-500'}>{driver.company}</span>
                        </div>
                      </div>
                    </div>
                        ...
                        <div className="flex items-center gap-2">
                          <button 
                            ...
                            className={`px-6 py-2 rounded-xl font-black text-[10px] uppercase transition-all shadow-md active:scale-95 shrink-0
                              ${isMorning 
                                ? (isDarkMode ? 'bg-white text-blue-950 hover:bg-blue-50' : 'bg-white text-blue-600 hover:bg-blue-50')
                                : (isDarkMode ? 'bg-white text-emerald-950 hover:bg-emerald-50' : 'bg-white text-emerald-600 hover:bg-emerald-50')
                              }`}
                          >
                            SAÍDA
                          </button>
...
const ContextButton: React.FC<{ onClick: () => void; icon: string; label: string; color?: string; isDarkMode: boolean }> = ({ onClick, icon, label, color, isDarkMode }) => (
  <button 
    onClick={onClick} 
    className={`flex items-center gap-4 px-6 py-4 text-left font-bold text-[10px] uppercase tracking-widest transition-colors ${
      isDarkMode ? 'bg-slate-900 hover:bg-white/5 text-white' : 'bg-white hover:bg-slate-100 text-slate-800'
